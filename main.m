...
for gen=1:maxgen
    ...
    for i=1:sizepop-sizemem
        ...
        if cost_exc>cost_sel %&& rand>exp(-a*b*c*(cost_exc-cost_sel)/cost_sel)  %模拟退火(exp里值绝对值越小越接受新解)
    ...
    for i=1:sizepop-sizemem
        ...
        if cost_cro>cost_exc %&& rand>exp(-a*b*c*(cost_cro-cost_exc)/cost_exc)  %模拟退火(exp里值绝对值越小越接受新解)
    ...
    for i=1:sizepop-sizemem
        ...
        if cost_rev>cost_cro %&& rand>exp(-a*b*c*(cost_rev-cost_cro)/cost_cro)  %模拟退火
    ...
    for i=1:sizepop-sizemem
        ...
        if cost_mut>cost_rev %&& rand>exp(-a*b*c*(cost_mut-cost_rev)/cost_rev)  %模拟退火
