...
for i=1:sizepop
    [cost,~]=Cost_Reward(plane,target,antibody(i,:));
...
for gen=1:maxgen
    ...
    for i=1:sizepop-sizemem
        [cost_sel,~]=Cost_Reward(plane,target,antibody_select(i,:));
        [cost_exc,~]=Cost_Reward(plane,target,antibody_exchange(i,:));
        if cost_exc>cost_sel && rand>exp(-a*b*(cost_exc-cost_sel)/cost_sel)  %模拟退火(exp里值绝对值越小越接受新解)
            antibody_exchange(i,:)=antibody_select(i,:);
        end
        [cost_exc,mission]=Cost_Reward(plane,target,antibody_exchange(i,:));
    ...
    for i=1:sizepop-sizemem
        [cost_cro,~]=Cost_Reward(plane,target,antibody_cross(i,:));
        [cost_rev,~]=Cost_Reward(plane,target,antibody_reverse(i,:));
        if cost_rev>cost_cro && rand>exp(-a*b*(cost_rev-cost_cro)/cost_cro)  %模拟退火
            antibody_reverse(i,:)=antibody_cross(i,:);
        end
        [cost_rev,mission]=Cost_Reward(plane,target,antibody_reverse(i,:));
    ...
    for i=1:sizepop-sizemem
        [cost_rev,~]=Cost_Reward(plane,target,antibody_reverse(i,:));
        [cost_mut,~]=Cost_Reward(plane,target,antibody_mutate(i,:));
        if cost_mut>cost_rev && rand>exp(-a*b*(cost_mut-cost_rev)/cost_rev)  %模拟退火
            antibody_mutate(i,:)=antibody_reverse(i,:);
        end
        [cost_mut,mission]=Cost_Reward(plane,target,antibody_mutate(i,:));
    ...
    for i=1:sizepop
        [cost,mission]=Cost_Reward(plane,target,antibody(i,:));
        affinity(i)=1/(cost+Bias);
        if affinity(i)>best_aff
            best_mission=mission;
            best_aff=affinity(i);
...
disp('最佳任务分配');
disp(best_mission);
disp(['最小代价:',num2str(Cost_Reward(plane,target,best_mission)+CostBias(plane,target,best_mission)-Reward(plane,target,best_mission))])
